...
class poe_model_sig(nn.Module):
    ...
    def __init__(self, sig_vars, sig_mean, gene_intersect, patch_r):

        super(poe_model_sig, self).__init__()

        
        self.sig_vars_prior = torch.Tensor(sig_vars).to(device) # variance of signature/pure spots
        self.sig_mean_prior = torch.Tensor(sig_mean).to(device) 
        self.c_bn = len(self.sig_vars_prior) # latent number, numbers of bottle neck
        self.c_in = len(gene_intersect) # number of genes 
        ...
        self.POE_dec_rna = nn.Sequential(nn.Linear(128, 512, bias=True),
