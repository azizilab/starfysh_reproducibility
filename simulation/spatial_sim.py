...
def sample(meta_df, cell_type, k_cells=9, specs=None):
    ...
    if specs != None:
        ...
        assert (set(np.intersect1d(meta_df[cell_type], specs)) == set(
...
def simulate_signatures(ct_level, cell_types):
    if ct_level == 'major':
        ...
        # TODO: update the function sample (line 28): only allow cell types from minor_dict.keys() if we perform random sampling
...
def main():
    # TODO:
    #  (1). Add CID44971 directories as the default values in `sc_path`, `spatial_path` (assume `CID44971_TNBC` under `data` directory
    #  (2). Complete simulating signatures: if `major`, we subset from `data/major_signatures/.csv`; if `minor`, we subset from `data/1013*.csv`
    #  (3). If simulating from `minor` cell types, update cell type labels to Starfysh (dict values)
    #  (4). "Merge" the proportions from Wu et al. to Starfysh
    #     (e.g. ['PVL immature s1', 'PVL immature s2'] -> 'PVL immature', we update spatial densities & ground-truth proportions accordingly
